...
describe('GET /api/v1/culture', () => {
  it('returns culture sites in standard envelope', async () => {
  ...
  it('resolves i18n keys — titles are actual text, not keys', async () => {
    ...
    data.forEach((s) => {
      expect(s.title).not.toMatch(/^homepage\./);
    });
  });
  ...
  it('respects limit param', async () => {
    ...
    expect(payload.meta).toMatchObject({ limit: 1 });
  });
