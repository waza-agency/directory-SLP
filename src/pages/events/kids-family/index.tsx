...
interface KidsFamilyEventsProps {
  events: Event[];
}
...
export const getStaticProps: GetStaticProps = async ({ locale }) => {
  try {
    ...
    // Get kids-family events
    const { data: eventsData, error } = await supabase
      .from('events')
      .select('*')
      .eq('category', 'kids-family')
      .gte('end_date', safetyDateString)
      .order('start_date', { ascending: true });

    if (error) throw error;

    console.log('Kids & Family events found:', eventsData?.length || 0);
...
export default function KidsFamilyEvents({ events }: KidsFamilyEventsProps) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative bg-gradient-to-r from-cyan-600 to-blue-600 text-white">
        <div className="absolute inset-0">
          <Image
            src="/images/event-categories/kids-family.jpeg"
            alt="Kids & Family Events"
            fill
        ...
        <div className="container mx-auto px-4 py-16 relative">
          <div className="max-w-3xl">
            <h1 className="text-5xl font-bold mb-6">Kids & Family Events</h1>
            <p className="text-xl mb-8">
              Discover interactive workshops, shows, and activities designed for children and families in San Luis Potosí. Create unforgettable memories together!
            </p>
            <div className="flex gap-4">
              <Link
                href="#featured-events"
                className="bg-white text-cyan-600 px-6 py-3 rounded-full font-semibold hover:bg-cyan-50 transition-colors"
              >
          ...
          <section id="featured-events" className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredEvents.map((event) => (
                <Link
                  key={event.id}
                  href={`/events/kids-family/${event.id}`}
                  className="group"
                >
                  <div className="bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                    ...
                    <div className="p-6">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="bg-cyan-100 text-cyan-800 text-xs font-semibold px-2.5 py-0.5 rounded-full">
                          Featured
                        </span>
                      </div>
                      <h2 className="text-xl font-bold mb-3 group-hover:text-cyan-600 transition-colors">
                        {event.title}
                      </h2>
                      ...
                      <div className="mt-6 flex items-center justify-between">
                        ...
                        <button className="bg-cyan-600 text-white px-4 py-2 rounded-full hover:bg-cyan-700 transition-colors flex items-center gap-2">
          ...
          <div className="text-center py-16 bg-white rounded-xl shadow-sm">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-600 mb-6">
                There are no upcoming kids & family events at the moment. Check back later for new workshops and activities!
              </p>
              <Link
                href="/"
                className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-full text-white bg-cyan-600 hover:bg-cyan-700 transition-colors"
              >
                Return to Home
              </Link>
            </div>
